    ...
    public static void main(String[] args) {
     ...
     // for(int s=0; s < prob.size();s++){
        ...
        //IOperatorSelector operatorSelector = new AdaptivePursuit(operators, 0.8, 0.8,0.1);
        	//	ProbabilityMatching(operators, 0.8, 0.1);//(operators,alpha,pmin)
        ...
        //create AOS
        AOSStrategy aosStrategy = new AOSStrategy(creditAssignment3,creditAssignment3, operatorSelector);
        ...
        while (!instAlgorithm.isTerminated() && 
            ...
            //	System.out.println("generation=" + gen);
               //PopulationIO.writeObjectives(new File("output16/Popsize"+populationSize[k]+"/DTLZ2_"+ obj +"/archive_gen" + gen +"_seed"+i+".txt"), aos.getArchive());
               //PopulationIO.writeObjectives(new File("output16/Popsize"+populationSize[k]+"/WFG1_"+ obj +"/archive_gen" + gen +"_seed"+i+".txt"), aos.getArchive());
               //PopulationIO.writeObjectives(new File("output16/Popsize"+populationSize[k]+"/ZDT4/archive_gen" + gen +"_seed"+i+".txt"), aos.getArchive());
            	PopulationIO.writeObjectives(new File("output/test/ZDT4/test_gen"+gen+"_seed"+i+".txt"), aos.getPopulation());
